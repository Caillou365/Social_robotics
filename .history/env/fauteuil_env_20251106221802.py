...
class FauteuilEnv(gym.Env):
    ...
    def render(self):
        print(f"Position du but : {self.goal_pos}")  # Affiche la position du but
    print(f"Position du fauteuil : {self.robot_pos}")  # Affiche la position du fauteuil
        self.screen.fill((255, 255, 255))  # Fond blanc

        # Dessine le but (cercle vert) - Vérifie les coordonnées
        pygame.draw.circle(
            self.screen,
            (0, 255, 0),  # Couleur verte
            (int(self.goal_pos[0] * 50) + 300, int(self.goal_pos[1] * 50) + 300),  # Centre à (300, 300)
            10  # Rayon du cercle
        )

        # Dessine les obstacles (cercles bleus)
        for obj in self.objects:
            pygame.draw.circle(
                self.screen,
                (0, 0, 255),
                (int(obj["pos"][0] * 50) + 300, int(obj["pos"][1] * 50) + 300),
                int(obj["radius"] * 50)
            )

        # Dessine les personnes (cercles rouges)
        for human in self.humans:
            pygame.draw.circle(
                self.screen,
                (255, 0, 0),
                (int(human[0] * 50) + 300, int(human[1] * 50) + 300),
                20  # Rayon pour les personnes
            )
        ...
        pygame.draw.rect(
            self.screen,
            (0, 0, 0),
            (int(self.robot_pos[0] * 50) + 275, int(self.robot_pos[1] * 50) + 275, 50, 50)
        )
