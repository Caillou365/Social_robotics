...
class FauteuilEnv(gym.Env):
    def __init__(self, config):
        ...
        # Personnes avec direction et durée
        self.humans = [
            {"pos": np.array([2.0, 5.0]), "direction": np.random.uniform(-1, 1, size=2), "duration": 0},
            {"pos": np.array([7.0, 8.0]), "direction": np.random.uniform(-1, 1, size=2), "duration": 0}
        ]
    ...
    def reset(self, seed=None, options=None):
        self.robot_pos = np.array([1.0, 1.0], dtype=np.float32)
        # Réinitialise les directions et durées des personnes
