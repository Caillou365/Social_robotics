...
class FauteuilEnv(gym.Env):
    def __init__(self, config):
        ...
        # Génère un nombre aléatoire d'humains et d'obstacles (entre 2 et 15)
        num_humans = np.random.randint(2, 16)
        num_objects = np.random.randint(2, 16)

        # Ajuste pour ne pas dépasser 20 éléments au total
        while num_humans + num_objects > 20:
            num_humans = np.random.randint(2, 16)
            num_objects = np.random.randint(2, 16)
        ...
        # Génère les humains aléatoirement
        self.humans = []
